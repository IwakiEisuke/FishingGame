...
public class FootIK : MonoBehaviour
{
    ...
    private void SetFootIK(AvatarIKGoal part, float positionWeight, float rotationWeight)
    {
        ...
        if (Physics.SphereCast(ray, _footRadius, out var hit, _rayDistance, _layerMask))
        {
            if (Physics.Raycast(hit.point + Vector3.up, Vector3.down, out hit, _rayDistance, _layerMask))
            {
